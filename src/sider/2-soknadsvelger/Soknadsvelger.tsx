...
class Soknadsobjekter extends Component<MergedProps> {
  ...
  hasSoknadsDialog() {
    ...
    return ((this.props.valgtUnderkategori.inngangtilsoknadsdialog || {}).soknadsdialogURL || {})[
    ...
    switch (soknader.status) {
      ...
      case "RESULT":
        ...
        if (this.hasSoknadsDialog()) {
          utlisting.push(
            `${
              intl.formatMessage({id: "vissoknadsobjekter.tittel"})
            } ${
              localeTekst(valgtUnderkategori.navn, intl.locale).toLowerCase()
            }`);
        }
        utlisting = utlisting.concat(soknader.soknadsobjekter.map(soknadsobjekt =>
          `${localeTekst(soknadsobjekt.navn, intl.locale)} - ${soknadsobjekt.hovedskjema.skjemanummer}`
        ));
        utlisting = utlisting.concat(soknader.skjemalenker.map(skjemalenke =>
          `${localeTekst(skjemalenke.navn, intl.locale)} - ${skjemalenke.hovedskjema.skjemanummer}`
        ));
        utlisting = utlisting.concat(soknader.soknadslenker.map(soknadslenke =>
          `${localeTekst(soknadslenke.navn, intl.locale)}`
        ))
        return (
          <>
            <Helmet>
